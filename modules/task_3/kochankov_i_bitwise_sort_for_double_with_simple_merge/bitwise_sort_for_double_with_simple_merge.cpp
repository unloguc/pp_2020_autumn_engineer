...
std::vector<double> parallel_bitwise_sort(const std::vector<double>& vect) {
   ...
   while (process_num_map.size() > 1) {
      int index = std::find(process_num_map.begin(), process_num_map.end(), make_pair(rank, count)) - process_num_map.begin();
      ...
      if (index % 2 == 0) {
         ...
         // cout << "Process " << rank << " ready to revice the message with length: "<< process_num_map[index -1].second << flush << endl;

         vector<double> tmp(process_num_map[index - 1].second);
         MPI_Recv(&tmp[0], process_num_map[index - 1].second, MPI_DOUBLE, process_num_map[index -1].first, 0, MPI_COMM_WORLD, &status);
