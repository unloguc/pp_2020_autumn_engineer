...
TEST(method_Seidel, test_evkl_norm) {
    ...
    ASSERT_NEAR(fct(test), sqrt(91), 1e-5);
}

TEST(method_Seidel, sequental_method) {
    const int size = 3;
    ...
    b[2] = 14;
    x = Seq(A, b, size, 1e-5);
    ASSERT_NEAR(fct(x), fct(tmp), 1e-5);
}

TEST(method_Seidel, parallel_method_1) {
    const int size = 3;
    std::vector<std::vector<double> > A(size);
    for (int i = 0; i < size; ++i) {
        A[i] = std::vector<double>(size);
...
TEST(method_Seidel, parallel_method_2) {
    const int size = 3;
    std::vector<std::vector<double> > A(size);
    for (int i = 0; i < size; ++i) {
        A[i] = std::vector<double>(size);
...
TEST(method_Seidel, comparison_of_parallel_and_sequental_methods) {
    const int size = 20;
    const int low = 0;
    const int high = 100;
    std::vector<std::vector<double> > A(size);
    ...
    if (rank == 0) {
        ...
        sequential = Seq(A, b, size, 1e-5);
    }
    parallel = Par(A, b, size, 1e-5);
    if (rank == 0) {
        ASSERT_NEAR(fct(sequential), fct(parallel), 1e-5);
    }
}
