...
#include <vector>
#include <numeric>
#include "../../../modules/task_2/ndah_r_interative_method/interative_method.h"
using std::vector;

double fct(const vector<double>& x) {
    double norm = 0;
...
vector<double> Seq(const vector<vector<double> >& A, const vector<double>& b, int n, double eps) {
    vector<double> x(n, 0);
    ...
    } while (fct(tmp) / fct(prev_x) > eps);
    return x;
}

vector<double> Par(const vector<vector<double> >& A, const vector<double>& _b, int _n, double eps) {
