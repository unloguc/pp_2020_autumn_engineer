...
vector<double> parallel_simple_iteration(const Matrix &matrix, double accuracy)
{
    ...
    // cout << "Process " << rank << " get subMat: Matrix(" << local_matrix.get_rows() << "," << local_matrix.get_columns() << ")" << endl;
    // cout << "[";
    ...
    while (true)
    {
        ...
        for (int i = 0; i < local_matrix.get_rows(); i++)
        {
            curent_vars_values[i] = local_matrix[i][local_matrix.get_columns() - 1] / local_matrix[i][i + rank * delta];

            for (int j = 0; j < local_matrix.get_columns() - 1; j++)
            {
                if (i + rank * delta != j)
                {
                    // cout << "Process " << rank << " was : " << curent_vars_values[i] << std::flush << endl;
                    curent_vars_values[i] -= (local_matrix[i][j] / local_matrix[i][i + rank * delta]) * prev_vars_values[j];
                }
            }
        }
        ...
        MPI_Allgatherv(&curent_vars_values[0], count, MPI_DOUBLE, &prev_vars_values[0], &resive_counts[0], &dips[0], MPI_DOUBLE, MPI_COMM_WORLD);
